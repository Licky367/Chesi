...
/* =========================
   COMMENT / POST HELPERS
========================= */
async function addComment({ dairyId, userId, comment }) {
  if (!dairyId || !userId || !comment) {
    throw new Error('Dairy, user, and comment are required');
  }
...
async function updateImage({ dairyId, userId, image }) {
  if (!dairyId || !userId || !image) {
    throw new Error('Dairy, user, and image are required');
  }
...
async function createPost({ dairyId, userId, userName, text, image }) {
  if (!dairyId || !userId || !userName) {
    throw new Error('Dairy, user, and username are required');
  }
...
async function toggleLike({ postId, userId }) {
  if (!postId || !userId) {
    throw new Error('Post and user are required');
  }

  const post = await Update.findOne({ _id: postId, type: 'post' });
...
async function addPostComment({ postId, userId, userName, text }) {
  if (!postId || !userId || !userName || !text) {
    throw new Error('Post, user, username, and text are required');
  }
...
async function deletePost({ postId, user }) {
  ...
  await Update.deleteOne({ _id: postId });
  return true;
}

async function deleteComment({ commentId, user }) {
  if (!commentId || !user) {
    throw new Error('Comment and user are required');
  }

  const post = await Update.findOne({ 'comments._id': commentId, type: 'post' });
  if (!post) return false;
...
async function getPositiveDairies() {
  return Dairy.find({ isMilking: true }).sort({ code: 1 }).lean();
}

async function getNegativeDairies() {
  return Dairy.find({ code: { $lt: 0 } }).sort({ code: 1 }).lean();
}
...
exports.getPositiveDairies = getPositiveDairies;
exports.getNegativeDairies = getNegativeDairies;
